  ...
  try {
    ...
    if (!imageBuffer || imageBuffer.length === 0) {
      ...
      if (!fallbackBuffer || fallbackBuffer.length === 0) {
        return NextResponse.json({ error: 'Failed to generate graph image' }, { status: 500 });
      }
...
async function takeScreenshot(lensHandle: string): Promise<Buffer> {
  ...
  try {
    ...
    browser = await puppeteer.launch({
      ...
      executablePath: await chromium.executablePath(), // Use Sparticus Chromium
      args: [
        ...chromium.args, // Include serverless-optimized args
        '--no-sandbox', // Required for Vercel
        '--disable-setuid-sandbox',
        '--disable-dev-shm-usage', // Prevent memory issues
        '--disable-gpu', // Disable GPU for headless
        '--single-process', // Optimize for serverless
      ],
    ...
    await page
      .waitForSelector('#screenshot-ready-indicator', {
        timeout: 5000,
      })
      .catch((err) => {
